...
class App:
    ...
    def _build_calibration_tab(self):
        ...
        self.notebook.add(tab, text="📷 相机标定")

        #  工具栏 =====
        toolbar = ttk.Frame(tab)
        toolbar.pack(fill=tk.X, pady=(0, 4))
        ttk.Button(toolbar, text="🔌 连接相机", command=self._connect_camera, width=10).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="▶ 开始预览", command=self._start_streaming, width=10).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="📷 采集标定", command=self._capture_calib, width=10).pack(side=tk.LEFT, padx=1)
        self.cal_cnt = ttk.Label(toolbar, text="0/10", foreground="#aaa", font=("", 9))
        self.cal_cnt.pack(side=tk.LEFT, padx=6)
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=6)
        ttk.Label(toolbar, text="曝光:", font=("", 8)).pack(side=tk.LEFT)
        ...
        ttk.Label(toolbar, text="增益:", font=("", 8)).pack(side=tk.LEFT, padx=(6, 0))
        ...
        ttk.Button(toolbar, text="应用", command=self._apply_cam_params, width=5).pack(side=tk.LEFT, padx=4)
        ...
        ttk.Label(right_frame, text="📋 图像列表", font=("", 9, "bold")).pack(anchor="w", pady=(0, 2))
        ...
        ttk.Button(cal_btns, text="执行标定", command=self._run_calibration, width=10).pack(side=tk.LEFT, padx=1)
        ttk.Button(cal_btns, text="保存标定", command=self._save_calibration, width=10).pack(side=tk.LEFT, padx=1)
        ttk.Button(cal_btns, text="加载标定", command=self._load_calibration, width=10).pack(side=tk.LEFT, padx=1)
        ttk.Button(cal_btns, text="重置", command=self._reset_calibration, width=8).pack(side=tk.LEFT, padx=1)

        ttk.Label(right_frame, text="标定结果", font=("", 9, "bold")).pack(anchor="w", pady=(4, 2))
    ...
    def _build_detection_tab(self):
        tab = ttk.Frame(self.notebook, padding=4)
        self.notebook.add(tab, text="🔍 划痕检测")

        #  ????? (??, ??) =====
        toolbar = ttk.Frame(tab)
        toolbar.pack(fill=tk.X, pady=(0, 4))
        self.det_start_btn = ttk.Button(toolbar, text="▶ 开始检测", command=self._start_detection, width=10)
        self.det_start_btn.pack(side=tk.LEFT, padx=1)
        self.det_stop_btn = ttk.Button(toolbar, text="⏹ 停止检测", command=self._stop_detection, width=10, state=tk.DISABLED)
        self.det_stop_btn.pack(side=tk.LEFT, padx=1)
        self.det_pause_btn = ttk.Button(toolbar, text="⏸ 暂停", command=self._toggle_pause, width=7, state=tk.DISABLED)
        self.det_pause_btn.pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="📸 截图", command=self._screenshot, width=8).pack(side=tk.LEFT, padx=1)
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=6)

        # 参数
        self.param_vars = {}
        params = [
            ("gamma", "伽马", 5, 25, int(SCRATCH_CFG.gamma * 10)),
            ("canny_low", "Canny低", 1, 255, SCRATCH_CFG.canny_low),
            ("canny_high", "Canny高", 1, 255, SCRATCH_CFG.canny_high),
            ("min_contour_length", "最小周长", 5, 500, int(SCRATCH_CFG.min_contour_length)),
            ("area_threshold", "最小面积", 10, 500, int(SCRATCH_CFG.area_threshold)),
            ("scratch_aspect_ratio_min", "最小长宽比", 1, 20, int(SCRATCH_CFG.scratch_aspect_ratio_min * 10)),
            ("max_scratch_count", "最大轮廓数", 5, 100, int(SCRATCH_CFG.max_scratch_count)),
        ]
        for name, label, lo, hi, default in params:
            ...
            s.pack()
        ttk.Button(toolbar, text="应用参数", command=self._apply_params, width=8).pack(side=tk.LEFT, padx=4)
        ...
        ttk.Label(lf, text="📹 实时图像 (金属板)", font=("", 9, "bold")).pack(anchor="w")
        ...
        ttk.Label(rf, text="🔍 划痕检测结果", font=("", 9, "bold")).pack(anchor="w")
        ...
        self.det_detail.insert(1.0, "等待检测结果...")
    ...
    def _connect_camera(self):
        ...
        try:
            ...
            if self.camera.open():
                # 自动加载已保存的 XML 相机参数（如果存在）
                xml_path = os.path.join(os.path.dirname(__file__), "camera_settings.xml")
                if os.path.exists(xml_path):
                    self.camera.load_xml_settings(xml_path)
                    # XML 可能覆盖了 Mono8 设置，重新确保灰度模式
                    self.camera._set_feature("PixelFormat", "Mono8")
    ...
    def _update_frame(self):
        ...
        if cur == 0:
            ...
            if self.detecting and not self._paused:
                t0 = time.time()
                ...
                self._last_views = views  # 保存最后一帧结果
            elif self.detecting and self._paused and self._last_views is not None:
                # 暂停时显示上次检测结果
                self._show_on_canvas(self.det_result, self._last_views["result"], "_det_result_tk")
            else:
    ...
    def _show_on_canvas(self, canvas, img, ref):
        ...
        if len(resized.shape) == 2:
            # 显示前做对比度拉伸，避免全白/全黑
            lo, hi = np.percentile(resized, [2, 98])
            if hi - lo > 10:
                stretched = cv2.normalize(resized, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
            else:
                stretched = resized
    ...
    def _capture_calib(self):
        ...
        if ok:
            ...
            raw_path = os.path.join(CALIB_IMAGES_DIR, f"calib_{n:02d}_{ts}_original.png")
            det_path = os.path.join(CALIB_IMAGES_DIR, f"calib_{n:02d}_{ts}_detected.png")
            safe_imwrite(raw_path, frame)
            ...
            self._set_status("标定图像 {} 已采集 ({})".format(n, ts))
        else:
    ...
    def _run_calibration(self):
        ...
        if ok:
            ...
            msg = "RMS: {} px\n质量: {}\n焦距: fx={}, fy={}\n主点: cx={}, cy={}".format(
                info["rms_error_px"], info["rms_error_px"],
    ...
    def _show_calib_result(self, info):
        ...
        lines.append("=" * 50)
        lines.append("  RMS: {} px".format(info["rms_error_px"], info["rms_error_px"]))
        lines.append("")
        lines.append("  [内参]")
        ...
        lines.append("  畸变: k1={}, k2={}".format(d["k1"], d["k2"]))
        lines.append("         p1={}, p2={}".format(d["p1"], d["p2"]))
        lines.append("")
        lines.append("  相机矩阵:")
        ...
        lines.append("  " + "-" * 34)
        for ext in info.get("extrinsics", []):
            t = ext["translation_mm"]
            dist = (t[0]**2 + t[1]**2 + t[2]**2)**0.5
            lines.append("  #{:02d}  | {:11.1f}  | {:.4f}".format(
                ext["image"], dist, ext["error_px"]))
        lines.append("")
        lines.append("  最佳: #{} | 最差: #{}".format(info.get("best_index", 0), info.get("worst_index", 0)))
        lines.append("")
        cov = info.get("coverage", {})
        if cov:
            lines.append("  [覆盖分析]")
            lines.append("  水平覆盖: {}% | 垂直覆盖: {}%".format(
                cov.get("coverage_x_pct", "?"), cov.get("coverage_y_pct", "?")))
            lines.append("  X范围: [{:.0f}, {:.0f}] | Y范围: [{:.0f}, {:.0f}]".format(
                cov["x_range"][0], cov["x_range"][1],
                cov["y_range"][0], cov["y_range"][1]))
            lines.append("")
        hints = info.get("improvement_hints", [])
        if hints:
            lines.append("  [优化建议]")
            for h in hints:
                lines.append("  - " + h)
    ...
    def _start_detection(self):
        ...
        self.det_pause_btn.config(state=tk.NORMAL)
    ...
    def _stop_detection(self):
        ...
        self.det_pause_btn.config(state=tk.DISABLED)
        self._set_status("检测已停止")

    def _toggle_pause(self):
        self._paused = not self._paused
        if self._paused:
            self.det_pause_btn.config(text="▶ 继续")
            self._set_status("检测已暂停")
        else:
            self.det_pause_btn.config(text="⏸ 暂停")
            self._set_status("检测继续")
    ...
    def _apply_params(self):
        ...
        for name, var in self.param_vars.items():
            if hasattr(cfg, name):
                val = var.get()
                if name == "gamma" or name == "scratch_aspect_ratio_min":
                    val = val / 10.0
                elif name in hints and hints[name] == int:
                    val = int(val)
